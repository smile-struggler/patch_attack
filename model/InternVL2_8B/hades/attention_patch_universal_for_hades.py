...
if __name__ == "__main__":
    ...
    parser.add_argument('--result_path', 
                        default='/workshop/crm/project/patch_attack/images/patch_noise_image/hades', type=str)
    ...
    target_file = '/workshop/crm/project/patch_attack/hades_goals-and-targets_detailed.json'
    result = {}
    ...
    # # 攻击类型列表
    # attack_types = ['full', 'top', 'bottom', 'right', 'right_blank']
    # attack_types = ['right']

    scenario = {}
    for item in tqdm(hades):
        if item['step'] != 5:
            continue
        
        if item['scenario'] not in scenario:
            scenario[item['scenario']] = 0
        
        scenario[item['scenario']]+=1
        ...
        if scenario[item['scenario']] <= 10:
            question_list.append(new_inst)
            target_list.append(target_dict[inst])

            image = item['image'].convert('RGB').resize((int(1024 / 1324 * 448), 448))
            ...
            scale_image_list.append(no_resize_transform(image).unsqueeze(0).to(torch.bfloat16).cuda())
            image_list.append(load_image(item['image'], resize_shape=(448, 448)))
    ...
    for attack_type in attack_types:
        ...
        if attack_type in ['top', 'bottom']:
            patch_height = int(300/1324 * 448)
            patch_width = 448

        elif attack_type in ['right', 'right_blank']:
            patch_height = 448
            patch_width = 448 - int(1024/1324 * 448)

        # 初始化对抗样本或补丁
        if attack_type == 'full':
            # 第一种攻击：从全零图像开始
            adv_example = torch.zeros((1, 3, *image_size),device = model.device, dtype=ori_images.dtype, requires_grad=True)
        elif attack_type in ['top', 'bottom', 'right', 'right_blank']:
        ...
        for i in range(num_steps):
            ...
            if attack_type == 'full':
            ...
            elif attack_type == 'top':
                # 将补丁应用到每张图像
                x_start, y_start = 0, 0
                adv_images = ori_images.clone()
                adv_patch_expanded = adv_example.expand(batch_size, -1, -1, -1)
                adv_images[:, :, x_start:x_start+patch_height, y_start:y_start+patch_width] = adv_patch_expanded

            elif attack_type == 'bottom':
                x_start, y_start = 448 - patch_height, 0
                adv_images = ori_images.clone()

                region = ori_images[:, :, x_start:x_start+patch_height, y_start:y_start+patch_width]
                white_mask = is_white(region, max_values)
                white_mask = white_mask.unsqueeze(1).expand(-1, ori_images.shape[1], -1, -1)
                
                # 将补丁应用到图像
                adv_patch_expanded = adv_example.expand(batch_size, -1, -1, -1)
                adv_images[:, :, x_start:x_start+patch_height, y_start:y_start+patch_width] = torch.where(
                    white_mask,
                    adv_patch_expanded,
                    region
                )
                ...
                # 将patch拼到图像的右侧
                x_start, y_start = 0, int(1024/1324 * 448)
                adv_images = scale_images_tensor.clone()
                adv_patch_expanded = adv_example.expand(batch_size, -1, -1, -1)
                adv_images = torch.cat((adv_images, adv_patch_expanded), dim=3)
                if attack_type == 'right_blank':
                    adv_images[:, :, 0:y_start, 0:y_start] = 0
        ...
        result_image = result_image.byte()
        img = Image.fromarray(result_image.permute(1, 2, 0).cpu().numpy(), 'RGB')
        img.save(os.path.join(args.result_path, f'hades_internvl2_patch_total{total_img_attn_weight}_entropy{attn_entropy_weight}_{attack_type}_{args.annotation}.png'))
